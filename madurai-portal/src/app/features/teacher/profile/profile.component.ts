...
import { TeacherProfileDto } from '../../../shared/models/profile.model';
...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  loadProfile(): void {
    ...
    this.profileService.getMyProfile().subscribe({
      next: (data) => {
        this.profile = data as TeacherProfileDto;
      ...
      error: () => {
        this.loading = false;
        const cached = localStorage.getItem('teacherProfile');
        if (cached) {
          this.profile = JSON.parse(cached);
          if (this.profile) this.patchForms(this.profile);
        }
        this.snackBar.open('Could not load profile. Showing cached data.', 'Close', { duration: 4000 });
      }
    });
  }
  ...
  private patchForms(p: TeacherProfileDto): void {
    this.personalForm.patchValue({
      firstName: p.firstName || '', lastName: p.lastName || '',
      preferredName: p.preferredName || '', gender: p.gender || '',
      dateOfBirth: p.dateOfBirth || '', maritalStatus: p.maritalStatus || ''
    });
    this.contactForm.patchValue({
      primaryMobile: p.primaryMobile || '', alternateMobile: p.alternateMobile || '',
      personalEmail: p.personalEmail || '', permanentAddress: p.permanentAddress || '',
      emergencyContactName: p.emergencyContactName || '',
      emergencyContactRelationship: p.emergencyContactRelationship || '',
      emergencyContactPhone: p.emergencyContactPhone || ''
    });
    this.churchForm.patchValue({
      homeChurch: p.homeChurch || '', churchDenomination: p.churchDenomination || '',
      baptized: p.baptized || false, ministryExperience: p.ministryExperience || '',
      ministryInterests: p.ministryInterests || '', callingTestimony: p.callingTestimony || ''
    });
  ...
  savePersonal(): void {
    ...
    this.saving = true;
    this.profileService.updateMyTeacherProfile(this.personalForm.value).subscribe({
      next: (u) => { this.profile = u; this.saving = false; this.snackBar.open('Personal information saved.', 'Close', { duration: 4000 }); },
      error: () => { this.saving = false; this.snackBar.open('Failed to save.', 'Close', { duration: 4000 }); }
    });
  }
  ...
  saveContact(): void {
    ...
    this.saving = true;
    this.profileService.updateMyTeacherProfile(this.contactForm.value).subscribe({
      next: (u) => { this.profile = u; this.saving = false; this.snackBar.open('Contact information saved.', 'Close', { duration: 4000 }); },
      error: () => { this.saving = false; this.snackBar.open('Failed to save.', 'Close', { duration: 4000 }); }
    });
  }
  ...
  saveChurch(): void {
    ...
    this.saving = true;
    this.profileService.updateMyTeacherProfile(this.churchForm.value).subscribe({
      next: (u) => { this.profile = u; this.saving = false; this.snackBar.open('Church & Ministry information saved.', 'Close', { duration: 4000 }); },
      error: () => { this.saving = false; this.snackBar.open('Failed to save.', 'Close', { duration: 4000 }); }
    });
  }
  ...
  savePreferences(): void {
    this.saving = true;
    this.profileService.updateMyTeacherProfile(this.preferencesForm.value).subscribe({
      next: (u) => { this.profile = u; this.saving = false; this.snackBar.open('Preferences saved.', 'Close', { duration: 4000 }); },
      error: () => { this.saving = false; this.snackBar.open('Failed to save.', 'Close', { duration: 4000 }); }
    });
  }
 ...
 onFileSelected(event: Event): void {
  ...
  changePassword(): void {
    ...
    this.profileService.changePassword({ currentPassword, newPassword, confirmPassword }).subscribe({
      next: (res) => { this.saving = false; this.passwordForm.reset(); this.snackBar.open(res.message, 'Close', { duration: 4000 }); },
      error: (err) => { this.saving = false; this.snackBar.open(err?.error?.message || 'Failed to change password.', 'Close', { duration: 4000 }); }
    });
  }
